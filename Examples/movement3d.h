void movement3D(Window* win, Camera3D* cam, float delta)
{
	...
	cam->T.Position += cam->GetDirection() * movement.z;
	cam->T.Position += cam->GetUpDirection() * movement.y;
	cam->T.Position += cam->GetRightDirection() * movement.x;
	...
	cam->Direction = dir.Value({ deg2rad(pitch), deg2rad(yaw) });
